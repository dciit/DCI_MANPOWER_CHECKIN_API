...
namespace API_DCI_DIAGRAM_SVG.Controllers
{
    public class SvgController : Controller
    {
        ...
        public SvgController(DBSCM contextSCM, DBDCI contextDCI, ManpowerContext contxMP, HRMContext contxHRM)
        {
        ...
        public IActionResult GetObjectListByLayout([FromBody] MParamLayoutInfo param)
        {
            ...
            //var oEmpMQs = _contextDCI.ViTrTrainessLog.Where(tr => tr.EmpCode == "I2524" && tr.Result == "P" && tr.Status == "post").ToList().GroupBy(g => g.MqNo);
            Service serv = new Service(_contextDCI,_contxMP,_contxHRM, _contextSCM);
        ...
        public IActionResult GetObjectInfoByCode([FromBody] MParamObjectCodeInfo param)
        {
            ...
            if (oObjects.Count > 0)
            {
                ...
                // FIND MQ & SA OF OBJECT 




                //if (oSAs.Count > 0 || oMQs.Count > 0)
                //{
                //    List<string> oMan = new List<string>();
                //    foreach (MpckDictionary oMQ in oMQs.ToList())
                //    {
